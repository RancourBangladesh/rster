...
export default function AdminRosterDataModal({ open, onClose, headers, teams, onUpdateShift, selectedDate, shiftDefinitions }: Props) {
  const SHIFT_DEFS = shiftDefinitions || SHIFT_MAP;
  
  // Get shift options from definitions (exclude empty string key)
  const SHIFT_OPTIONS = useMemo(() => {
    return Object.keys(SHIFT_DEFS).filter(code => code !== '');
  }, [SHIFT_DEFS]);
  ...
  const { displayHeaders, displayMonthName, displayDates, canGoPrev, canGoNext } = useMemo(() => {
    ...
    // Allow dynamic navigation - not restricted to available months only
    // Allow navigation within reasonable bounds (e.g., 5 years back/forward)
    const currentYear = new Date().getFullYear();
    const canGoPrev = year > currentYear - 5;
  ...
  function displayShift(code: string): string {
    if (!code) return '-';
    // If shift is not defined, show it with a warning
    if (!SHIFT_DEFS[code]) {
      return `⚠️ ${code}`;
    }
    return SHIFT_DEFS[code] || code;
  }
  ...
  const legendItems = SHIFT_OPTIONS.map((code: string) => ({ code, label: SHIFT_DEFS[code] || code }));
    ...
    <Modal open={open} onClose={onClose} title="Admin Roster Data" width="95vw">
      <div style={{padding: 20}}>
        ...
        <div style={{display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12}}>
          <span style={{fontWeight: 700, color: 'var(--text-dim)'}}>Team Filters:</span>
          {teamNames.map(t => (
            <button
              key={t}
        ...
        <div style={{display:'flex', gap:16, alignItems:'flex-start', justifyContent:'space-between', marginBottom:12, flexWrap:'wrap'}}>
          ...
          <div style={{
            ...
            <div style={{fontSize:'.8rem', color:'var(--text-dim)', marginBottom:6, fontWeight:600}}>Shift Mapping</div>
            <div style={{
              display:'grid',
              gridTemplateColumns:'repeat(4, minmax(0, 1fr))',
              gap:'6px 8px'
            }}>
              {legendItems.map((item: {code: string; label: string}) => (
          ...
          <div style={{flex:'0 0 320px', display:'flex', gap:8, alignItems:'center'}}>
            <input
              ...
              style={{padding:'9px 12px', border:'1px solid var(--border)', borderRadius:8, background:'var(--bg)', color:'var(--text)', width:'100%'}}
            />
              ...
              {filteredEmployees.map(emp => (
                <tr key={emp.id}>
                  ...
                  {headerIndexByDisplay.map((hdrIdx) => {
                    ...
                    const isDeletedShift = shift && !SHIFT_DEFS[shift];
                    return (
                      <td
                        key={`${emp.id}-${hdrIdx}`}
                        className={`adm-cell ${shift ? 'has' : ''} ${isActive ? 'active' : ''} ${isDeletedShift ? 'deleted-shift' : ''}`}
                        onClick={(e)=>handleCellClick(e, emp.id, hdrIdx)}
                        title={isDeletedShift ? "Click to update - this shift code has been deleted" : "Click to edit shift"}
                      >
          ...
          {editingCell && (
            ...
            >
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                {SHIFT_OPTIONS.map((s: string) => (
                  <button key={s} className="btn tiny" onClick={() => handleShiftSelect(s)} title={SHIFT_DEFS[s] || s}>
                    {s}
                  </button>
                ))}
      ...
      <style jsx>{`
        ...
        .sv-chip.active {
          background:#3b82f6; border-color:#2563eb; color:#fff; box-shadow:0 0 0 2px rgba(59,130,246,.28);
        }
        ...
        .adm-cell.has { background: transparent; }
        .adm-cell:hover { background: #f3f4f6; }
        .adm-shift { font-size: .92rem; }
        
        /* Deleted shift indicator */
        .adm-cell.deleted-shift {
          background: #fef3c7 !important;
          border: 1px solid #fbbf24;
        }
        .adm-cell.deleted-shift:hover {
          background: #fde68a !important;
        }
        .adm-cell.deleted-shift .adm-shift {
          color: #b45309;
          font-weight: 600;
        }
