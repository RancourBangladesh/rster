  ...
  return (
    <Modal open={open} onClose={onClose} title={modalTitle} width="95vw">
              ...
              <div className="rtm-suggestions">
                {suggestions.map(s=>(
                  <button key={s.id} className="sv-chip" onClick={()=> setSelectedEmpIds(prev => prev.includes(s.id) ? prev : [...prev, s.id])}>
                    <Plus size={14} style={{display:'inline', marginRight:'4px'}} /> {s.name} ({s.id})
                  </button>
                ))}
              </div>
            )}
      ...
      <style jsx>{`
        ...
        .rtm-intro { margin:0 0 6px; color: var(--text-dim); }
        ...
        .sv-chip { padding: 6px 12px; font-size: .85rem; border-radius: 8px; background: var(--panel-alt); border: 1px solid var(--border); color: var(--text); cursor: pointer; }
        .sv-chip.active { background:var(--primary); border-color:var(--primary); color:#fff; box-shadow:0 0 0 2px rgba(79, 70, 229, 0.2); }
        ...
        .rtm-legend-title { font-size:.8rem; color:var(--text-dim); margin-bottom:6px; font-weight:600; }
        .rtm-legend-grid { display:grid; grid-template-columns: repeat(4, minmax(0,1fr)); gap: 6px 8px; }
        .rtm-legend-cell { border:1px solid var(--border); border-radius:8px; padding:6px 8px; font-size:.78rem; color:var(--text); background:var(--panel-alt); display:flex; gap:6px; align-items:center; min-height: 32px; }
        ...
        .rtm-search input { padding:9px 12px; border:1px solid var(--border); border-radius:8px; background:var(--bg); color:var(--text); width:100%; }
        .rtm-suggestions, .rtm-selected { width:100%; display:flex; gap:6px; flex-wrap:wrap; }

        .rtm-month-row { display:flex; align-items:center; justify-content:space-between; background: var(--panel); border:1px solid var(--border); border-radius:8px; padding:10px; }
        .rtm-month-left { display:flex; align-items:center; gap:15px; }
        .rtm-month-name { font-size:1.1rem; }
        .rtm-month-info { display:flex; gap:15px; font-size:.9rem; color:var(--text-dim); }
        ...
        .rtm-table { width: max-content; min-width: 100%; border-collapse: separate; border-spacing: 0; table-layout: fixed; color: var(--text); }
        .rtm-table thead th, .rtm-table tbody td { border-bottom: 1px solid var(--border); border-right: 1px solid var(--border); padding: 10px 8px; background: transparent; white-space: nowrap; vertical-align: middle; }
        .rtm-table thead th:first-child, .rtm-table tbody td:first-child { border-left: 1px solid var(--border); }
        ...
        .rtm-head-day { font-size: .72rem; font-weight: 800; letter-spacing: .3px; color: var(--text); }
        .rtm-head-date { font-size: .7rem; color: var(--text-dim); }
        ...
        .rtm-emp-meta { font-size:.8rem; color:var(--text-dim); }
        .rtm-team-pill { margin-left:8px; padding:2px 6px; background: var(--primary); color:#fff; border-radius:4px; font-size:.75rem; }
        ...
        .rtm-cell.active { outline: 2px solid var(--primary); outline-offset: -2px; z-index: 1; }
        ...
        .rtm-add-title { font-size:.85rem; color: var(--text-dim); font-weight: 600; }
        ...
        .rtm-team-card { background: var(--panel); border-radius: 12px; padding: 30px; min-width: 350px; box-shadow: 0 8px 24px rgba(0,0,0,.4); }
        .rtm-team-current { color: var(--text-dim); margin-bottom: 12px; }
        .rtm-team-list { display:flex; flex-direction:column; gap:8px; }
        .rtm-team-list .btn { padding: 12px; text-align: left; background: var(--primary); color:#fff; }
