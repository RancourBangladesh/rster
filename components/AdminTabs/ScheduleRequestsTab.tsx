...
export default function ScheduleRequestsTab({id}:Props) {
  ...
  async function load() {
    setLoading(true);
    try {
      const pendRes = await fetch('/api/schedule-requests/get-pending', { cache: 'no-store' }).then(r=>r.json()).catch(()=>null);
      if (pendRes?.success) {
        setStats((prev:any)=>({...prev, ...pendRes.stats}));
      }
      const allRes = await fetch('/api/schedule-requests/get-all', { cache: 'no-store' }).then(r=>r.json()).catch(()=>null);
      if (allRes?.success) {
