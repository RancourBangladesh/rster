...
import { BarChart3, Users, CheckCircle, XCircle, Clock, Edit, UserCheck, RefreshCw } from 'lucide-react';
  ...
  return (
    <div id={id} className="tab-pane">
      ...
      <div className="dashboard-grid">
        ...
        >
          <h3><UserCheck size={20} style={{display:'inline', marginRight:'8px', verticalAlign:'middle'}} /> Employees Working Today</h3>
          <div className="big-stat">
        ...
        <div className="dashboard-card">
          <h3><RefreshCw size={20} style={{display:'inline', marginRight:'8px', verticalAlign:'middle'}} /> Shift Change / Swap Requests Overview</h3>
          ...
          {showRequestsDetails && (
            <div className="requests-details" style={{
              ...
              <h4 style={{margin: '0 0 10px', color: 'var(--text)', fontSize: '1rem'}}>
                          ...
                          <table style={{width: '100%', fontSize: '0.85rem'}}>
                            <thead>
                              <tr style={{borderBottom: '1px solid var(--border)'}}>
                                <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Employee</th>
                                <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Date</th>
                                <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Change</th>
                                <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Modified By</th>
                              </tr>
                            </thead>
                            <tbody>
                              {modifications.slice(0, 10).map((m: any, idx: number) => (
                                <tr key={idx} style={{borderBottom: '1px solid var(--border)'}}>
                                  <td style={{padding: '8px', color: 'var(--text)'}}>{m.employee_name}</td>
                                  <td style={{padding: '8px', color: 'var(--text-dim)'}}>{m.date_header}</td>
                                  <td style={{padding: '8px', color: 'var(--primary)'}}>{m.old_shift} → {m.new_shift}</td>
                                  <td style={{padding: '8px', color: 'var(--text-dim)', fontSize: '0.8rem'}}>{m.modified_by}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                        ...
                        <table style={{width: '100%', fontSize: '0.85rem'}}>
                          <thead>
                            <tr style={{borderBottom: '1px solid var(--border)'}}>
                              <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Employee</th>
                              <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Date</th>
                              <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Type</th>
                              <th style={{padding: '8px', textAlign: 'left', color: 'var(--text-dim)'}}>Status</th>
                            </tr>
                          </thead>
                          <tbody>
                            {filteredRequests.slice(0, 10).map((req: any, idx: number) => (
                              <tr key={idx} style={{borderBottom: '1px solid var(--border)'}}>
                                ...
                                <td style={{padding: '8px', color: 'var(--text-dim)'}}>{req.date}</td>
                                <td style={{padding: '8px', color: 'var(--text-dim)'}}>{req.type === 'swap' ? 'Swap' : 'Shift Change'}</td>
              ...
              stats.recent_activity
                ...
                <div key={idx} className="activity-item">
                  <div className="activity-icon">
                    {activity.activity_type === 'modification' ? <Edit size={14} style={{display:'inline'}} /> : 
                     activity.status === 'approved' ? <CheckCircle size={14} style={{display:'inline', color:'var(--success)'}} /> : 
                     activity.status === 'rejected' ? <XCircle size={14} style={{display:'inline', color:'var(--danger)'}} /> : <Clock size={14} style={{display:'inline', color:'var(--warning)'}} />}
                  </div>
                  <div className="activity-content">
                    ...
                    <div className="activity-meta">
                      ...
                      {activity.activity_type === 'modification' ? (
                        <span className="activity-status" style={{background: 'var(--primary-light)', color: 'var(--primary)'}}>
                          modified
                        </span>
                      ) : (
        ...
        >
          ...
          >
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px'}}>
              <h3 style={{margin: 0, color: 'var(--text)'}}><UserCheck size={20} style={{display:'inline', marginRight:'8px', verticalAlign:'middle'}} /> Employees Working Today</h3>
              <button 
                onClick={() => setShowWorkingTodayModal(false)}
                style={{
                  background: 'none',
            ...
            <div style={{marginBottom: '20px'}}>
              <label style={{fontSize: '0.9rem', color: 'var(--text-dim)', marginBottom: '8px', display: 'block'}}>
                Filter by Team:
              </label>
              <div style={{display: 'flex', gap: '8px', flexWrap: 'wrap'}}>
                {Object.keys(stats.team_stats).map(team => (
                  <button
                    ...
                    style={{
                      padding: '6px 12px',
                      background: selectedTeamsFilter.includes(team) ? 'var(--primary)' : 'var(--panel-alt)',
                      border: '1px solid var(--border)',
                      borderRadius: '6px',
                      color: selectedTeamsFilter.includes(team) ? '#fff' : 'var(--text)',
                      cursor: 'pointer',
                      fontSize: '0.85rem'
                    }}
                  >
            ...
            <div style={{maxHeight: '400px', overflowY: 'auto'}}>
              {stats.working_today
                ...
                <table style={{width: '100%', borderCollapse: 'collapse'}}>
                  <thead>
                    <tr style={{borderBottom: '1px solid var(--border)'}}>
                      <th style={{padding: '10px', textAlign: 'left', color: 'var(--text-dim)', fontSize: '0.85rem'}}>Name</th>
                      <th style={{padding: '10px', textAlign: 'left', color: 'var(--text-dim)', fontSize: '0.85rem'}}>ID</th>
                      <th style={{padding: '10px', textAlign: 'left', color: 'var(--text-dim)', fontSize: '0.85rem'}}>Team</th>
                      <th style={{padding: '10px', textAlign: 'left', color: 'var(--text-dim)', fontSize: '0.85rem'}}>Shift</th>
                    </tr>
                  </thead>
                  <tbody>
                    {stats.working_today
                      ...
                      .map((emp, idx) => (
                        <tr key={idx} style={{borderBottom: '1px solid var(--border)'}}>
                          <td style={{padding: '10px', color: 'var(--text)', fontSize: '0.9rem'}}>{emp.name}</td>
                          <td style={{padding: '10px', color: 'var(--text-dim)', fontSize: '0.85rem'}}>{emp.id}</td>
                          <td style={{padding: '10px', color: 'var(--text-dim)', fontSize: '0.85rem'}}>{emp.team}</td>
                          <td style={{padding: '10px', color: 'var(--primary)', fontSize: '0.9rem', fontWeight: '600'}}>{emp.shift}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
