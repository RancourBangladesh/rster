...
export function updateShiftForTenant(tenantId: string, employeeId: string, dateIndex: number, newShift: string, googleOriginal: string, modifiedBy: string) {
...
export function applyShiftChangeRequestForTenant(tenantId: string, req: any, approvedBy: string) {
  const admin = getAdminForTenant(tenantId);
  const idx = admin.headers.indexOf(req.date);
  updateShiftForTenant(
    tenantId,
    req.employee_id,
    idx,
    req.requested_shift,
...
export function applySwapForTenant(tenantId: string, req: any, approvedBy: string) {
  const admin = getAdminForTenant(tenantId);
  const idx = admin.headers.indexOf(req.date);
  if (idx === -1) return;
